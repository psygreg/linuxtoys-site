class ToolsCacheLoader {
  ...
  async loadCache() {
    try {
      const response = await fetch(
        `${this.cacheBaseUrl}translated-categories.json`,
      );
      if (!response.ok) {
      ...
      console.warn(
        "Could not load tools cache, falling back to static data:",
        error,
      );
      return false;
    }
  ...
  createCategoryCard(categoryData, categoryConfig) {
    ...
    button.className =
      "mt-auto bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300 toggle-button";
    button.setAttribute("data-key", "toggle-button");

    // Apply correct translation immediately
    if (
      window.translationManager &&
      typeof window.translationManager.getTranslation === "function"
    ) {
      button.textContent =
        window.translationManager.getTranslation("toggle-button");
    } else {
  ...
  async init() {
    ...
    if (updated) {
      ...
      if (summary) {
        console.log(
          `Tools data last updated: ${new Date(summary.lastUpdated).toLocaleDateString()}`,
        );
        console.log(
          `Total categories: ${summary.categoryCount}, Total tools: ${summary.totalTools}`,
        );
      }
    } else {
    ...
    newToggleButtons.forEach((button) => {
      ...
      if (list && list.classList.contains("expanded")) {
        if (
          ...
          button.textContent =
            window.translationManager.getTranslation("toggle-button-less");
        } else {
