...
- VLC
- Gnome Tweaks
- OBS Studio (incluye el plugin [Pipewire Audio Capture](https://github.com/dimtpap/obs-pipewire-audio-capture)), si la máquina tiene una GPU Intel (discreta o integrada). De lo contrario, se instalará como un flatpak a nivel de usuario desde Flathub.
...
### Corrección GTK para Battlemage
Soluciona problemas con la renderización de aplicaciones GTK con GPUs Intel Arc serie B (*Battlemage*) cambiándolas al modo OpenGL. Se puede revertir simplemente eliminando la línea añadida usando un editor de texto como `nano`.
