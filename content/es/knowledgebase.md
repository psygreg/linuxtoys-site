...
- VLC
- Gnome Tweaks
- OBS Studio (incluye el plugin [Pipewire Audio Capture](https://github.com/dimtpap/obs-pipewire-audio-capture) y `v4l2loopback` para compatibilidad con Cámara Virtual), si la máquina tiene una GPU Intel (discreta o integrada) y no ejecuta sistemas basados en Arch. De lo contrario, se instalará como un flatpak a nivel de usuario desde Flathub (sin GPU Intel), desde el [AUR](https://aur.archlinux.org/packages/obs-studio-browser) debido a la falta de una fuente de navegador en el paquete predeterminado de Arch Linux, o desde su repositorio PPA oficial para Ubuntu.
- Nala
...
- Kisak-Mesa: de su repositorio PPA oficial, versión "fresh".
- OpenLinkHub: de sus repositorios oficiales PPA, COPR y AUR. Asegúrese de guardar la página web abierta después de la instalación en sus marcadores, ya que es el panel de control.
...
- Faugus Launcher: desde Flathub; su repositorio AUR oficial si está en Arch Linux y sistemas derivados; o repositorio COPR para Fedora y derivados.
- Positron: desde sus paquetes oficiales para sistemas compatibles.
- RStudio: desde sus paquetes oficiales para sistemas compatibles.
- Adoptium JDK: desde sus paquetes oficiales para sistemas compatibles.
- Astah Pro: desde sus paquetes oficiales para sistemas compatibles.
...
No tiene ningún empaquetado actualizado en los repositorios aparte de NixOS (unstable), por lo que LinuxToys lo instala o actualiza en `$HOME/.local/share/stoat` desde el archivo *zip* proporcionado en el repositorio oficial de GitHub. Se puede eliminar simplemente borrando este directorio.

### Tac Writer

Instalado usando el script gentilmente proporcionado por el desarrollador en su [repositorio de GitHub](https://github.com/narayanls/tac-writer) como un paquete estándar a través de su gestor de paquetes.
...
Habilita la codificación de hardware VAAPI para tarjetas gráficas que no sean Nvidia en Linux a través de FFMpeg. Se instala copiando el tarball extraído de la última versión disponible en su [repositorio oficial de GitHub](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) a `/opt/resolve/IOPlugins/`. **ADVERTENCIA:** si usa *DaVinciBox* debe instalar LinuxToys dentro del contenedor y ejecutar este instalador desde allí para que esta función funcione como se espera.
...
Instala `fish` desde los repositorios predeterminados, y `fisher` con el instalador proporcionado en su [repositorio oficial](https://github.com/jorgebucaran/fisher).

**Paquetes Instalados o Actualizados**
- Todos los sistemas: `fish`
...
### Corrección de Bubblewrap
Resuelve un problema que se encuentra comúnmente en ciertas versiones de Ubuntu donde AppArmor bloquea Bubblewrap, lo que provoca que las aplicaciones flatpak y Steam dejen de funcionar. 

**Configuraciones personalizadas aplicadas**
- En `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### Corrección de KDE Connect
Resuelve un problema común que causa que KDE Connect o GSConnect no logren localizar ningún dispositivo en su red, causado por tener los puertos que requiere para conectar dispositivos entre sí bloqueados por el cortafuegos.

**Configuraciones personalizadas aplicadas**
- Abre los puertos 1714-1764 (TCP y UDP) al tráfico de red local de forma adaptativa según su cortafuegos activo actualmente (`firewalld`, `ufw` e `iptables`).

### Memoria Libre Mínima Dinámica
Establece el valor de la configuración del kernel `vm.min_free_kbytes` dinámicamente de acuerdo con el total de RAM disponible, haciendo que haya más RAM disponible para otras tareas en dispositivos con memoria limitada. También es adaptativo al arrancar, lo que significa que no tienes que reinstalarlo si realizas cambios en tu hardware.

**Configuraciones personalizadas aplicadas**
- Crea el servicio systemd `/etc/systemd/system/set-min-free-mem.service`:
```
[Unit]
...
### Terra
Instalado siguiendo su documentación. Solo disponible para Fedora y derivados.
