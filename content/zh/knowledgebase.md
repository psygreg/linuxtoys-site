...
- VLC
- Gnome Tweaks
- OBS Studio（包含[Pipewire Audio Capture](https://github.com/dimtpap/obs-pipewire-audio-capture)插件和`v4l2loopback`用于虚拟摄像头兼容性），如果机器具有Intel GPU（独立或集成）并且不运行基于Arch的系统。否则，它将作为来自Flathub的用户级flatpak安装（无Intel GPU），来自[AUR](https://aur.archlinux.org/packages/obs-studio-browser)（由于默认Arch Linux软件包中缺少浏览器源），或从其适用于 Ubuntu 的官方 PPA 存储库中安装。
- Nala
...
- Kisak-Mesa：来自其官方 PPA 存储库，“fresh” 版本。
- OpenLinkHub：来自其官方 PPA、COPR 和 AUR 仓库。安装后请务必将打开的网页保存到您的书签中，因为它是控制面板。
...
- Faugus Launcher：来自 Flathub；如果在 Arch Linux 及其衍生系统上，则来自其官方 AUR 仓库；对于 Fedora 及其衍生系统，则来自 COPR 仓库。
- Positron：来自其针对兼容系统的官方软件包。
- RStudio：来自其针对兼容系统的官方软件包。
- Adoptium JDK：来自其针对兼容系统的官方软件包。
- Astah Pro：来自其针对兼容系统的官方软件包。
...
除了 NixOS (unstable) 之外，在存储库上没有任何最新的软件包，因此它由 LinuxToys 根据官方 GitHub 仓库中提供的 *zip* 文件在 `$HOME/.local/share/stoat` 上安装或更新。只需删除此目录即可将其删除。

### Tac Writer

使用开发人员在其 [GitHub 仓库](https://github.com/narayanls/tac-writer) 中提供的脚本，通过您的包管理器作为标准包进行安装。
...
### 适用于 DaVinci Resolve 的 FFMpeg 编码器

通过 FFMpeg 在 Linux 上为非 Nvidia 显卡启用 VAAPI 硬件编码。通过将其官方 [GitHub 仓库](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) 上的最新版本提取出的 tarball 复制到 `/opt/resolve/IOPlugins/` 来安装。 **警告：** 如果使用 *DaVinciBox*，您必须在容器内安装 LinuxToys 并从中运行此安装程序，此功能才能按预期工作。
...
从默认存储库安装 `fish`，并使用其[官方仓库](https://github.com/jorgebucaran/fisher)提供的安装程序安装 `fisher`。
...
应用各种性能调整和常见问题的修复。虽然所有这些都会被安装，但许多只有在需要时才会激活 - 因为只有当它们针对的设备在您的系统中被发现时，它们才会启动。这允许您更改组件，为这些新部件动态应用补丁。对于原子 Fedora 和 Universal Blue 系统，这些将作为分层包安装，并可以通过 `rpm-ostree` 删除包来删除。对于其他系统，可以通过删除相应的文件来恢复。包和直接安装方法都使用直接从 *CachyOS* 存储库获取的文件。
...
### Bubblewrap 修复
解决了某些 Ubuntu 版本中常见的问题，即 AppArmor 阻止 Bubblewrap，导致 flatpak 应用程序和 Steam 停止工作。 

**应用的自定义设置**
- 在 `/etc/apparmor.d/bwrap`：
```
abi <abi/4.0>,
...
### KDE Connect 修复
解决了导致 KDE Connect 或 GSConnect 无法在您的网络中找到任何设备的常见问题，这是由于防火墙阻止了设备相互连接所需的端口。

**应用的自定义设置**
- 根据您当前活跃的防火墙（`firewalld`、`ufw` 或 `iptables`），自适应地向本地网络流量开放端口 1714-1764（TCP 和 UDP）。

### 动态最小空闲内存
根据可用总 RAM 动态设置 `vm.min_free_kbytes` 内核设置值，从而为内存受限设备中的其他任务腾出更多 RAM。它在启动时也是自适应的，这意味着如果您更改硬件，则无需重新安装。

**应用的自定义设置**
- 创建 systemd 服务 `/etc/systemd/system/set-min-free-mem.service`：
```
[Unit]
