...
- VLC
- Gnome Tweaks
- OBS Studio (includes [Pipewire Audio Capture](https://github.com/dimtpap/obs-pipewire-audio-capture) plugin and `v4l2loopback` for Virtual Camera compatibility), if the machine has an Intel GPU (discrete or integrated) and is not running Arch-based systems. Otherwise, it will be installed as a user-level flatpak from Flathub (no Intel GPU), from the [AUR](https://aur.archlinux.org/packages/obs-studio-browser) due to the lack of a browser source in the default Arch Linux package, or from its official PPA repository for Ubuntu.
- Nala
...
- Kisak-Mesa: from its official PPA repository, "fresh" version.
- OpenLinkHub: from their official PPA, COPR and AUR repositories. Make sure to save the webpage opened after installation in your bookmarks, as it is the control panel.
...
- Faugus Launcher: from Flathub; its official AUR repository if on Arch Linux and derivative systems; or COPR repository for Fedora and derivatives.
- Positron: from its official packages for compatible systems.
- RStudio: from its official packages for compatible systems.
- Adoptium JDK: from its official packages for compatible systems.
- Astah Pro: from its official packages for compatible systems.
...
Doesn't have any up-to-date packaging on repositories besides NixOS (unstable), so it's installed or updated by LinuxToys on `$HOME/.local/share/stoat` from the provided *zip* file in the official GitHub repository. Can be removed by simply deleting this directory.

### Tac Writer

Installed using the script gently provided by the developer at its [GitHub repository](https://github.com/narayanls/tac-writer) as a standard package through your package manager.
...
### FFMpeg encoder for DaVinci Resolve

Enables VAAPI hardware encoding for non-Nvidia graphics cards on Linux through FFMpeg. Installed by copying over the extracted tarball from the latest version available on its official [GitHub repository](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) to `/opt/resolve/IOPlugins/`. **WARNING:** if using *DaVinciBox* you must install LinuxToys within the container and run this installer from it for this feature to work as expected.
...
**Packages Installed or Updated for DaVinciBox**
...
Installs `fish` from default repositories, and `fisher` with the installer provided at its [official repository](https://github.com/jorgebucaran/fisher).
...
### AppArmor Setup for Debian and Arch Linux
...
### Bubblewrap Fix
Solves an issue typically found in certain Ubuntu versions where AppArmor blocks Bubblewrap, causing flatpak applications and Steam to stop working. 

**Custom settings applied**
- At `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### KDE Connect Fix
Solves a common issue causing KDE Connect or GSConnect to fail to locate any devices in your network, caused by having the ports it requires to connect devices to each other blocked by the firewall.

**Custom settings applied**
- Opens ports 1714-1764 (TCP and UDP) to local network traffic adaptively according to your currently active firewall (`firewalld`, `ufw` or `iptables`).

### Dynamic Minimum Free Memory
Sets `vm.min_free_kbytes` kernel setting value dynamically according to the total RAM available, making more RAM available for other tasks in memory-constrained devices. It's also adaptive on boot, meaning you don't have to reinstall it if you make changes to your hardware.

**Custom settings applied**
- Creates systemd service `/etc/systemd/system/set-min-free-mem.service`:
```
[Unit]
...
### Terra
Installed following their documentation. Only available for Fedora and derivatives.
