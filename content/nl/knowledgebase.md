...
- Kisak-Mesa: vanuit de officiële PPA-repository, "fresh" versie.
- OpenLinkHub: van hun officiële PPA, COPR en AUR repositories. Zorg ervoor dat u de webpagina die na de installatie wordt geopend opslaat in uw bladwijzers, aangezien dit het controlepaneel is.
...
- Faugus Launcher: van Flathub; zijn officiële AUR-repository op Arch Linux en afgeleide systemen; of COPR-repository voor Fedora en afgeleiden.
- Positron: van de officiële pakketten voor compatibele systemen.
- RStudio: van de officiële pakketten voor compatibele systemen.
- Adoptium JDK: van de officiële pakketten voor compatibele systemen.
- Astah Pro: van de officiële pakketten voor compatibele systemen.
...
Heeft geen actuele pakketten in de repositories behalve voor NixOS (unstable), dus het wordt geïnstalleerd of bijgewerkt door LinuxToys op `$HOME/.local/share/stoat` vanuit het meegeleverde *zip*-bestand in de officiële GitHub-repository. Het kan worden verwijderd door simpelweg deze map te verwijderen.

### Tac Writer

Geïnstalleerd met behulp van het script dat vriendelijk is verstrekt door de ontwikkelaar op de [GitHub-repository](https://github.com/narayanls/tac-writer) als een standaardpakket via uw pakketbeheerder.
...
### FFMpeg-encoder voor DaVinci Resolve

Maakt VAAPI-hardwarecodering mogelijk voor niet-Nvidia-videokaarten op Linux via FFMpeg. Geïnstalleerd door de uitgepakte tarball van de nieuwste beschikbare versie in de officiële [GitHub-repository](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) te kopiëren naar `/opt/resolve/IOPlugins/`. **WAARSCHUWING:** bij gebruik van *DaVinciBox* moet u LinuxToys in de container installeren en dit installatieprogramma van daaruit uitvoeren om deze functie naar verwachting te laten werken.
...
Installeert `fish` van de standaard repositories, en `fisher` met het installatieprogramma dat wordt geleverd in de [officiële repository](https://github.com/jorgebucaran/fisher).
...
### AppArmor-instellingen voor Debian en Arch Linux
...
### Bubblewrap Fix
Lost een probleem op dat doorgaans voorkomt in bepaalde Ubuntu-versies waarbij AppArmor Bubblewrap blokkeert, waardoor flatpak-applicaties en Steam niet meer werken. 

**Aangepaste instellingen toegepast**
- Op `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### KDE Connect Fix
Lost een veelvoorkomend probleem op waarbij KDE Connect of GSConnect geen apparaten in uw netwerk kan vinden, veroorzaakt doordat de poorten die nodig zijn om apparaten met elkaar te verbinden worden geblokkeerd door de firewall.

**Aangepaste instellingen toegepast**
- Opent poorten 1714-1764 (TCP en UDP) voor lokaal netwerkverkeer op adaptieve wijze, afhankelijk van uw momenteel actieve firewall (`firewalld`, `ufw` of `iptables`).

### Dynamisch minimaal vrij geheugen
Stelt de kernelinstelling `vm.min_free_kbytes` dynamisch in op basis van het totale beschikbare RAM-geheugen, waardoor er meer RAM beschikbaar komt voor andere taken op apparaten met beperkt geheugen. Het is ook adaptief bij het opstarten, wat betekent dat u het niet opnieuw hoeft te installeren als u wijzigingen aanbrengt in uw hardware.

**Aangepaste instellingen toegepast**
- Maakt systemd service `/etc/systemd/system/set-min-free-mem.service` aan:
```
[Unit]
...
### Terra
Geïnstalleerd volgens hun documentatie. Alleen beschikbaar voor Fedora en afgeleiden.
