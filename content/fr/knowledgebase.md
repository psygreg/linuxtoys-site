...
- Kisak-Mesa : de son dépôt PPA officiel, version « fresh ».
- OpenLinkHub : depuis leurs dépôts officiels PPA, COPR et AUR. Assurez-vous d'enregistrer la page web ouverte après l'installation dans vos favoris, car il s'agit du panneau de contrôle.
...
- Faugus Launcher : depuis Flathub ; son dépôt AUR officiel si vous êtes sur Arch Linux et systèmes dérivés ; ou dépôt COPR pour Fedora et dérivés.
- Positron : depuis ses paquets officiels pour les systèmes compatibles.
- RStudio : depuis ses paquets officiels pour les systèmes compatibles.
- Adoptium JDK : depuis ses paquets officiels pour les systèmes compatibles.
- Astah Pro : depuis ses paquets officiels pour les systèmes compatibles.
...
Ne dispose d'aucun paquet à jour sur les dépôts en dehors de NixOS (unstable), il est donc installé ou mis à jour par LinuxToys dans `$HOME/.local/share/stoat` à partir du fichier *zip* fourni dans le dépôt GitHub officiel. Il peut être supprimé en effaçant simplement ce répertoire.

### Tac Writer

Installé à l'aide du script aimablement fourni par le développeur sur son [dépôt GitHub](https://github.com/narayanls/tac-writer) en tant que paquet standard via votre gestionnaire de paquets.
...
### Encodeur FFMpeg pour DaVinci Resolve

Active l'encodage matériel VAAPI pour les cartes graphiques non-Nvidia sur Linux via FFMpeg. Installé en copiant l'archive tar extraite de la dernière version disponible sur son [dépôt GitHub](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) officiel vers `/opt/resolve/IOPlugins/`. **AVERTISSEMENT :** si vous utilisez *DaVinciBox*, vous devez installer LinuxToys à l'intérieur du conteneur et exécuter cet installateur depuis celui-ci pour que cette fonctionnalité fonctionne comme prévu.
...
Installe `fish` depuis les dépôts par défaut, et `fisher` avec l'installateur fourni sur son [dépôt officiel](https://github.com/jorgebucaran/fisher).

**Paquets Installés ou Mis à Jour**
- Tous les systèmes : `fish`
...
### Configuration d'AppArmor pour Debian et Arch Linux
...
### Correction de Bubblewrap
Résout un problème couramment rencontré dans certaines versions d'Ubuntu où AppArmor bloque Bubblewrap, empêchant les applications flatpak et Steam de fonctionner. 

**Paramètres personnalisés appliqués**
- À `/etc/apparmor.d/bwrap` :
```
abi <abi/4.0>,
...
### Correction de KDE Connect
Résout un problème courant empêchant KDE Connect ou GSConnect de localiser des appareils sur votre réseau, causé par le blocage des ports nécessaires à la connexion des appareils entre eux par le pare-feu.

**Paramètres personnalisés appliqués**
- Ouvre les ports 1714-1764 (TCP et UDP) au trafic réseau local de manière adaptative en fonction de votre pare-feu actuellement actif (`firewalld`, `ufw` ou `iptables`).

### Mémoire libre minimale dynamique
Définit la valeur du paramètre du noyau `vm.min_free_kbytes` de manière dynamique en fonction de la RAM totale disponible, rendant plus de RAM disponible pour d'autres tâches sur les appareils aux ressources limitées. Il est également adaptatif au démarrage, ce qui signifie que vous n'avez pas besoin de le réinstaller si vous modifiez votre matériel.

**Paramètres personnalisés appliqués**
- Crée le service systemd `/etc/systemd/system/set-min-free-mem.service` :
```
[Unit]
...
### Terra
Installé en suivant leur documentation. Uniquement disponible pour Fedora et dérivés.
