...
- VLC
- Gnome Tweaks
- OBS Studio (beinhaltet [Pipewire Audio Capture](https://github.com/dimtpap/obs-pipewire-audio-capture) Plugin und `v4l2loopback` für Virtual Camera Kompatibilität), wenn die Maschine über eine Intel GPU (diskret oder integriert) verfügt und keine Arch-basierten Systeme ausgeführt werden. Andernfalls wird es als Flatpak auf Benutzerebene von Flathub (keine Intel GPU) installiert, aus dem [AUR](https://aur.archlinux.org/packages/obs-studio-browser), da die Standard-Arch-Linux-Pakete keine Browser-Quelle haben, oder aus dem offiziellen PPA-Repository für Ubuntu.
- Nala
...
- Kisak-Mesa: aus seinem offiziellen PPA-Repository, "fresh" Version.
- OpenLinkHub: aus ihren offiziellen PPA-, COPR- und AUR-Repositories. Speichern Sie die nach der Installation geöffnete Webseite unbedingt in Ihren Lesezeichen, da dies das Control Panel ist.
...
- Faugus Launcher: von Flathub; sein offizielles AUR-Repository, wenn auf Arch Linux und Derivaten; oder COPR-Repository für Fedora und Derivate.
- Positron: aus seinen offiziellen Paketen für kompatible Systeme.
- RStudio: aus seinen offiziellen Paketen für kompatible Systeme.
- Adoptium JDK: aus seinen offiziellen Paketen für kompatible Systeme.
- Astah Pro: aus seinen offiziellen Paketen für kompatible Systeme.
...
Hat außer für NixOS (unstable) keine aktuellen Pakete in den Repositories, daher wird es von LinuxToys unter `$HOME/.local/share/stoat` aus der bereitgestellten *zip*-Datei im offiziellen GitHub-Repository installiert oder aktualisiert. Es kann durch einfaches Löschen dieses Verzeichnisses entfernt werden.

### Tac Writer

Installiert mit dem Skript, das freundlicherweise vom Entwickler in seinem [GitHub-Repository](https://github.com/narayanls/tac-writer) als Standardpaket über Ihren Paketmanager bereitgestellt wurde.
...
### FFMpeg-Encoder für DaVinci Resolve

Ermöglicht VAAPI-Hardware-Kodierung für Nicht-Nvidia-Grafikkarten unter Linux über FFMpeg. Die Installation erfolgt durch Kopieren des entpackten Tarballs der neuesten verfügbaren Version aus dem offiziellen [GitHub-Repository](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) nach `/opt/resolve/IOPlugins/`. **WARNUNG:** Wenn Sie *DaVinciBox* verwenden, müssen Sie LinuxToys innerhalb des Containers installieren und diesen Installer von dort aus ausführen, damit diese Funktion wie erwartet funktioniert.
...
Installiert `fish` aus den Standard-Repositories und `fisher` mit dem im [offiziellen Repository](https://github.com/jorgebucaran/fisher) bereitgestellten Installer.
...
### AppArmor-Setup für Debian und Arch Linux
Installiert und aktiviert ein grundlegendes AppArmor-Setup - die gleichen Standardeinstellungen wie Ubuntu. Für ein besser gehärtetes MAC sollten Sie sich [apparmor.d](https://github.com/roddhjav/apparmor.d) ansehen, das aus verschiedenen Gründen nicht automatisiert installiert werden kann und dessen Dokumentation Sie sorgfältig lesen müssen, um es zu verwenden.
...
**Angewendete benutzerdefinierte Einstellungen**
- AppArmor muss als MAC für das System über die Kernel-CMDLINE aufgerufen werden. Dies wird über `/etc/default/grub.d/99-apparmor.cfg` für GRUB-Benutzer durchgeführt:
```
GRUB_CMDLINE_LINUX_DEFAULT="${GRUB_CMDLINE_LINUX_DEFAULT} apparmor=1 security=apparmor"
```
- Oder `/etc/kernel/cmdline.d/99-apparmor.conf` für systemd-boot-Benutzer:
```
apparmor=1 security=apparmor
```
- `apparmor.service` wird auch aktiviert.

### Bubblewrap-Fix
Behebt ein Problem, das typischerweise in bestimmten Ubuntu-Versionen auftritt, bei dem AppArmor Bubblewrap blockiert, wodurch Flatpak-Anwendungen und Steam nicht mehr funktionieren. 

**Angewendete benutzerdefinierte Einstellungen**
- Unter `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### KDE Connect-Fix
Behebt ein häufiges Problem, bei dem KDE Connect oder GSConnect keine Geräte in Ihrem Netzwerk finden können, da die für die Verbindung der Geräte erforderlichen Ports durch die Firewall blockiert werden.

**Angewendete benutzerdefinierte Einstellungen**
- Öffnet die Ports 1714-1764 (TCP und UDP) adaptiv für den lokalen Netzwerkverkehr entsprechend Ihrer derzeit aktiven Firewall (`firewalld`, `ufw` oder `iptables`).

### Dynamischer minimaler freier Speicher
Legt den Wert der Kernel-Einstellung `vm.min_free_kbytes` dynamisch entsprechend dem verfügbaren Gesamtspeicher (RAM) fest, wodurch auf Geräten mit begrenztem Speicher mehr RAM für andere Aufgaben zur Verfügung steht. Es ist auch beim Booten adaptiv, was bedeutet, dass Sie es nicht neu installieren müssen, wenn Sie Änderungen an Ihrer Hardware vornehmen.

**Angewendete benutzerdefinierte Einstellungen**
- Erstellt den systemd-Dienst `/etc/systemd/system/set-min-free-mem.service`:
```
[Unit]
...
### Terra
Installiert gemäß ihrer Dokumentation. Nur für Fedora und Derivate verfügbar.
