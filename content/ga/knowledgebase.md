...
- Solaar: óna stóras PPA oifigiúil ar Ubuntu. Tá sé suiteáilte ó stórtha réamhshocraithe ag córais eile.
...
- Kisak-Mesa: óna stóras PPA oifigiúil, leagan "fresh".
- OpenLinkHub: óna stórtha oifigiúla PPA, COPR agus AUR. Déan cinnte an leathanach gréasáin a osclaíodh tar éis an tsuiteála a shábháil i do leabharmharcanna, mar gurb é an painéal rialaithe é.
...
- Faugus Launcher: ó Flathub; a stóras AUR oifigiúil má tá tú ar Arch Linux agus córais dhíorthacha; nó stóras COPR do Fedora agus díorthaigh.
- Positron: óna phacáistí oifigiúla do chórais chomhoiriúnacha.
- RStudio: óna phacáistí oifigiúla do chórais chomhoiriúnacha.
- Adoptium JDK: óna phacáistí oifigiúla do chórais chomhoiriúnacha.
- Astah Pro: óna phacáistí oifigiúla do chórais chomhoiriúnacha.
...
Níl aon phacáistí cothrom le dáta aige ar stórtha seachas NixOS (unstable), mar sin tá sé suiteáilte nó nuashonraithe ag LinuxToys ar `$HOME/.local/share/stoat` ón gcomhad *zip* a cuireadh ar fáil ar an stóras oifigiúil GitHub. Is féidir é a bhaint trí an t-eolaire seo a scriosadh go simplí.

### Tac Writer

Suiteáilte ag baint úsáide as an script a chuir an forbróir ar fáil go cineálta ina [stóras GitHub](https://github.com/narayanls/tac-writer) mar phacáiste caighdeánach trí do bhainisteoir pacáiste.
...
### Códeic Sruthaithe do Fedora/OpenSUSE
...
Ceartaíonn sé saincheist ina gcuireann SELinux cosc ar aon rud a rith trí WINE/Proton ar OpenSUSE. Is féidir é a aisiompú trí an t-ordú céanna a úsáid le `0` mar luach Boole in ionad `1`.
...
### Ionchódóir FFMpeg do DaVinci Resolve

Cumasaíonn sé ionchódú crua-earraí VAAPI do chártaí grafaice nach de chuid Nvidia iad ar Linux trí FFMpeg. Suiteáilte trí an tarball eastósctha ón leagan is déanaí atá ar fáil ar a [stóras GitHub](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) oifigiúil a chóipeáil go `/opt/resolve/IOPlugins/`. **RABHADH:** má tá *DaVinciBox* á úsáid agat caithfidh tú LinuxToys a shuiteáil laistigh den choimeádán agus an suiteálaí seo a rith as sin chun go n-oibreoidh an ghné seo mar a mbeifí ag súil leis.
...
**Pacáistí Suiteáilte nó Nuashonraithe do DaVinciBox**
...
Suiteáilann sé `fish` ó stórtha réamhshocraithe, agus `fisher` leis an tsuiteálaí a cuireadh ar fáil ina [stóras oifigiúil](https://github.com/jorgebucaran/fisher).
...
### Réiteach Bubblewrap
Réitíonn sé fadhb a fhaightear go hiondúil i leaganacha áirithe de Ubuntu nuair a chuireann AppArmor bac ar Bubblewrap, rud a fhágann go stopann feidhmchláir flatpak agus Steam ag obair. 

**Socruithe saincheaptha curtha i bhfeidhm**
- Ag `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### Réiteach KDE Connect
Réitíonn sé fadhb choitianta a fhágann nach n-aimsíonn KDE Connect nó GSConnect aon ghléasanna ar do líonra, de bharr go bhfuil na poirt a theastaíonn uaidh chun gléasanna a nascadh le chéile blocáilte ag an mballa dóite.

**Socruithe saincheaptha curtha i bhfeidhm**
- Osclaíonn sé poirt 1714-1764 (TCP agus UDP) do thrácht líonra áitiúil go hoiriúnaitheach de réir do bhalla dóite gníomhach faoi láthair (`firewalld`, `ufw` nó `iptables`).

### Cuimhne Shaor Íosta Dinimiciúil
Socraíonn sé luach socraithe eithne `vm.min_free_kbytes` go dinimiciúil de réir an iomlán RAM atá ar fáil, rud a fhágann go bhfuil níos mó RAM ar fáil do thascanna eile i ngléasanna a bhfuil teorainn le cuimhne acu. Tá sé oiriúnaitheach freisin ag am tosaithe, rud a chiallaíonn nach gcaithfidh tú é a athshuiteáil má dhéanann tú athruithe ar do chrua-earraí.

**Socruithe saincheaptha curtha i bhfeidhm**
- Cruthaíonn sé seirbhís systemd `/etc/systemd/system/set-min-free-mem.service`:
```
[Unit]
...
### Terra
Suiteáilte ag leanúint a ndoiciméadachta. Ar fáil do Fedora agus díorthaigh amháin.
...
Socrú aon-chliceáil a shuiteálann roghnúchán coimeádta, cobhsaí optamaithe do do chóras. Ní shuiteálfaidh sé gnéithe nach bhfuil ábhartha nó atá i láthair cheana i do mheaisín.
