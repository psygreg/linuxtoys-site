...
開始する前に、LinuxToysのライブラリと開発プラクティスに関する完全なドキュメントについて、[開発者ハンドブック](https://codeberg.org/psygreg/linuxtoys/wiki/Developer-Handbook)をご確認ください。
...
1. [開発者ハンドブック](https://codeberg.org/psygreg/linuxtoys/wiki/Developer-Handbook)をレビューしてください
...
- **`# negates:`** - このヘッダーを持つスクリプトがユーザーのマシンと互換性があることが判明した場合、以下のスクリプトを非表示にします。`.sh`拡張子を除いて、カンマで区切られた複数のスクリプトを指定できます。
...
LinuxToysを人々の母国語で利用できるようにすることは、ソフトウェアをよりアクセシブルにするため非常に重要であり、これは私たちの成功の重要な要因であることを理解しています。しかし、これは非常に時間のかかるタスクでもあり、私たちよりもはるかに大きなプロジェクトでさえ、これを手伝ってくれるボランティアを見つけるのに苦労しているため、翻訳の生成にAIツールを使用しています。翻訳の不正確さや間違いは、私たちの[GitHub issuesページ](https://codeberg.org/psygreg/linuxtoys/issues)で報告してください。
