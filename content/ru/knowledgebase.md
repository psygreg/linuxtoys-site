...
### Исправление GTK для Battlemage
Исправляет проблемы с рендерингом GTK-приложений с GPU Intel Arc серии B (*Battlemage*), переключая их в режим OpenGL. Можно отменить, просто удалив добавленную строку с помощью текстового редактора, такого как `nano`.
