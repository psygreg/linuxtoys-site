...
- Kisak-Mesa: из официального PPA-репозитория, версия «fresh».
- OpenLinkHub: из их официальных репозиториев PPA, COPR и AUR. Обязательно сохраните веб-страницу, открытую после установки, в закладках, так как она является панелью управления.
...
- Faugus Launcher: из Flathub; его официального репозитория AUR в Arch Linux и производных системах; или репозитория COPR для Fedora и производных.
- Positron: из его официальных пакетов для совместимых систем.
- RStudio: из его официальных пакетов для совместимых систем.
- Adoptium JDK: из его официальных пакетов для совместимых систем.
- Astah Pro: из его официальных пакетов для совместимых систем.
...
Не имеет актуальных пакетов в репозиториях, кроме NixOS (unstable), поэтому устанавливается или обновляется LinuxToys в `$HOME/.local/share/stoat` из предоставленного *zip*-файла в официальном репозитории GitHub. Его можно удалить, просто удалив этот каталог.

### Tac Writer

Устанавливается с помощью скрипта, любезно предоставленного разработчиком в его [репозитории GitHub](https://github.com/narayanls/tac-writer), как стандартный пакет через ваш менеджер пакетов.
...
### Кодировщик FFMpeg для DaVinci Resolve

Включает аппаратное кодирование VAAPI для видеокарт, отличных от Nvidia, в Linux через FFMpeg. Устанавливается путем копирования извлеченного tar-архива последней доступной версии из его официального [репозитория GitHub](https://github.com/EdvinNilsson/ffmpeg_encoder_plugin) в `/opt/resolve/IOPlugins/`. **ВНИМАНИЕ:** при использовании *DaVinciBox* вы должны установить LinuxToys внутри контейнера и запустить этот установщик из него, чтобы эта функция работала должным образом.
...
Устанавливает `fish` из стандартных репозиториев и `fisher` с помощью установщика, предоставленного в его [официальном репозитории](https://github.com/jorgebucaran/fisher).
...
### Установка AppArmor для Debian и Arch Linux
Устанавливает и включает базовую конфигурацию AppArmor - те же значения по умолчанию, что и Ubuntu. Для лучшей защищенной MAC следует обратить внимание на [apparmor.d](https://github.com/roddhjav/apparmor.d), который не может быть установлен автоматизированным способом по различным причинам, и вы должны внимательно прочитать его документацию, чтобы использовать его.
...
**Примененные пользовательские настройки**
- AppArmor должен быть вызван как MAC для системы через командную строку ядра. Это делается через `/etc/default/grub.d/99-apparmor.cfg` для пользователей GRUB:
```
GRUB_CMDLINE_LINUX_DEFAULT="${GRUB_CMDLINE_LINUX_DEFAULT} apparmor=1 security=apparmor"
```
- Или `/etc/kernel/cmdline.d/99-apparmor.conf` для пользователей systemd-boot:
```
apparmor=1 security=apparmor
```
- Сервис `apparmor.service` также включен.

### Исправление Bubblewrap
Решает проблему, обычно встречающуюся в некоторых версиях Ubuntu, когда AppArmor блокирует Bubblewrap, из-за чего приложения flatpak и Steam перестают работать. 

**Примененные пользовательские настройки**
- В `/etc/apparmor.d/bwrap`:
```
abi <abi/4.0>,
...
### Исправление KDE Connect
Решает распространенную проблему, из-за которой KDE Connect или GSConnect не могут обнаружить устройства в вашей сети, вызванную тем, что порты, необходимые для соединения устройств друг с другом, заблокированы брандмауэром.

**Примененные пользовательские настройки**
- Адаптивно открывает порты 1714-1764 (TCP и UDP) для трафика локальной сети в соответствии с вашим текущим активным брандмауэром (`firewalld`, `ufw` или `iptables`).

### Динамическая минимальная свободная память
Динамически устанавливает значение параметра ядра `vm.min_free_kbytes` в соответствии с общим объемом доступной оперативной памяти, освобождая больше памяти для других задач на устройствах с ограниченным объемом памяти. Он также адаптивен при загрузке, что означает, что вам не нужно переустанавливать его при изменении аппаратного обеспечения.

**Примененные пользовательские настройки**
- Создает службу systemd `/etc/systemd/system/set-min-free-mem.service`:
```
[Unit]
...
### Terra
Установлено согласно их документации. Доступно только для Fedora и производных.
